...
from stock_trading.experiments.lightgbm_strategy_freeze_executable import (
    FACTORY_SCHEMA,
...
def _report() -> dict:
    ...
    return {
        ...
        "generation": {"generation_id": "g002", "failed_hypotheses": 0},
...
def _qualified(variant_id: str, *, flagged: bool = False, ex_best_three: float = 0.01) -> dict:
    return {
...
def test_source_validation_requires_same_execution_realism() -> None:
    report = _report()
    qualification = _qualification(report)
    _validate_sources(report, qualification, "g002")

    changed = dict(qualification)
    changed["execution_realism"] = {
        **qualification["execution_realism"],
        "max_entry_day_participation_pct": 0.02,
    }
    with pytest.raises(ValueError, match="differs"):
        _validate_sources(report, changed, "g002")
