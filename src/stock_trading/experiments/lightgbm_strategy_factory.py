...
def _initialize_worker(common: dict[str, Any]) -> None:
    global _CONTEXT
    root = Path(common["experiment_dir"])
    source_rows = _load_training_rows(root / "training_rows.jsonl")
    market_store = DuckDbMarketStore(common["market_db"])
    market_store.enable_read_cache(max_series=int(common["market_read_cache_series"]))
    targets = build_multi_horizon_targets(
        source_rows,
        market_store,
        benchmark_security_id=common["benchmark_security_id"],
        horizons=(5, 20, 60),
...
def _feature_snapshots(rows, market_store: DuckDbMarketStore):
...
def _historical_candidates(rows, targets, market_store):
