...
from stock_trading.backtest.intelligent_portfolio import (
    IntelligentPortfolioBacktester,
...
class ProfitTargetV6ExperimentResult:
...
def run_profit_target_v6_experiment(
    ...
    *,
...
) -> ProfitTargetV6ExperimentResult:
    """Replay V5 selection exactly, then change only portfolio capital allocation.

    V6 deliberately does not retrain any predictor. Saved V5 5/20/60 profit and
    alpha heads are replayed with the same PIT calibration and horizon choice.
    The fixed-allocation replay must reproduce every V5 yearly trade count and
    return before the correlation/risk-aware portfolio layer is evaluated.
    """
    ...
    for required in (rows_path, v5_path):
        if not required.exists():
            raise FileNotFoundError(f"missing V6 prerequisite: {required}")
    ...
    if not splits:
        raise ValueError("no walk-forward splits available for V6 replay")
    ...
    intelligent_config = PortfolioIntelligenceConfig(
        starting_capital=starting_capital,
    ...
    intelligent_backtester = IntelligentPortfolioBacktester(
        market_store,
        intelligent_config,
    )
    ...
    for split in splits:
        ...
        if len(fixed_replay.trades) != int(expected_v5["trades"]):
            raise RuntimeError(
                f"V6 selection replay diverged from V5 trade count in {split.test_year}"
            )
        if not np.isclose(
            ...
            raise RuntimeError(
                f"V6 selection replay diverged from V5 return in {split.test_year}"
            )

        portfolio, diagnostics = intelligent_backtester.run(tuple(selected))
        ...
        year_reports.append(
            {
                ...
                "portfolio_intelligence": diagnostic_dict,
    ...
    observed = {
        ...
        "max_gross_exposure_pct": max(
            (float(item["portfolio_intelligence"]["max_gross_exposure_pct"]) for item in year_reports),
            default=0.0,
        ),
        "rejected_correlation_exposure": sum(
            int(item["portfolio_intelligence"]["rejected_correlation_exposure"])
            for item in year_reports
        ),
        "rejected_gross_exposure": sum(
            int(item["portfolio_intelligence"]["rejected_gross_exposure"])
            for item in year_reports
        ),
    }
    ...
    payload = _json_safe(
        {
            "schema_version": "profit-target-lightgbm-v6-portfolio-intelligence",
            ...
            "development_mode": {
                "kind": "portfolio_engineering",
                "predictor_retrained": False,
                "selection_identity_verified_against_v5": True,
                "reason": "add capital-allocation intelligence without changing V5 opportunity selection",
            },
            ...
            "portfolio_policy": asdict(intelligent_config),
            "observed": observed,
    ...
    output_path = root / "profit_target_v6_backtest.json"
    output_path.write_text(
        json.dumps(payload, indent=2, sort_keys=True, allow_nan=False),
        encoding="utf-8",
    )
    return ProfitTargetV6ExperimentResult(
        source_training_row_count=len(source_rows),
        complete_multi_horizon_row_count=len(rows),
...
def _weighted_diagnostic_average(
    ...
    weighted = [
        (count, float(values[key]))
        for count, values in rows
        if count > 0 and values.get(key) is not None
    ]
...
def _compound_excluding(years: list[dict], excluded: set[int]) -> float:
    return prod(
        1.0 + float(item["return"])
        for item in years
        if int(item["year"]) not in excluded
    ) - 1.0
...
def _parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Replay saved V5 adaptive-horizon selection, verify exact fixed-policy identity, "
            "then apply V6 dynamic sizing and PIT correlation-aware portfolio controls."
        )
    )
    ...
    parser.add_argument("--min-allocation-pct", type=float, default=0.0075)
    parser.add_argument("--max-allocation-pct", type=float, default=0.03)
    parser.add_argument("--max-gross-exposure-pct", type=float, default=0.35)
...
def main() -> None:
    ...
    result = run_profit_target_v6_experiment(
        args.experiment_dir,
